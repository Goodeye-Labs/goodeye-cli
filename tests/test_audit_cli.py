...
def test_audit_client_with_id_posts_workflow_route() -> None:
    route = respx.post(f"{SERVER}/v1/skills/wf_1/audit").mock(
    ...
    assert parsed.path == "/v1/skills/wf_1/audit"
...
def test_audit_client_without_id_gets_prompt_route() -> None:
    route = respx.get(f"{SERVER}/v1/audit/skill-prompt").mock(
        return_value=httpx.Response(
            200,
    ...
    parsed = urlparse(str(route.calls.last.request.url))
    assert parsed.path == "/v1/audit/skill-prompt"
...
def test_workflows_audit_command_with_id_prints_pack(
    ...
    respx.post(f"{SERVER}/v1/skills/wf_1/audit").mock(
        return_value=httpx.Response(
            200,
            json={
                "workflow_id": "wf_1",
